...
const actions = [
  ['add', 'Recargar'],
  ['information-circle-outline', 'Detalles'],
  ['lock-closed-outline', 'Bloquear'],
];

const management = [
  ['information-circle-outline', 'Ver PIN', 'Consulta de forma segura'],
  ['card-outline', 'Método de pago', 'Gestiona tus tarjetas'],
  ['shield-checkmark-outline', 'Seguridad', 'Límites y compras online'],
];
...
export default function CardsScreen() {
  return <Screen>
    ...
    <Pressable style={({ pressed }) => [styles.cardShell, pressed && styles.cardPressed]}>
      <LinearGradient colors={['#07124C', '#2C24B6', '#6858E9']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.bankCard}>
        ...
        <Text style={styles.cardType}>VISA · DÉBITO</Text>
        <Text style={styles.cardNumber}>••••  4209</Text>
        <View style={styles.cardBottom}><View><Text style={styles.cardMetaLabel}>TITULAR</Text><Text style={styles.cardMeta}>ARIAN MERCER</Text></View><View><Text style={styles.cardMetaLabel}>VENCE</Text><Text style={styles.cardMeta}>12/29</Text></View><Text style={styles.visa}>VISA</Text></View>
      </LinearGradient>
    </Pressable>
...
const styles = StyleSheet.create({
  ...
  eyebrow: { color: COLORS.blue, fontFamily: FONTS.button, fontSize: 9, letterSpacing: 1.2 },
  title: { marginTop: 3, color: COLORS.ink, fontFamily: FONTS.heading, fontSize: 25 },
  ...
  cardShell: { marginTop: 20, borderRadius: 27, shadowColor: '#253795', shadowOpacity: .38, shadowRadius: 17, shadowOffset: { width: 0, height: 12 }, elevation: 9 },
  cardPressed: { transform: [{ scale: .985 }], opacity: .96 },
  bankCard: { height: 210, borderRadius: 27, overflow: 'hidden', padding: 22, borderWidth: 1, borderColor: 'rgba(255,255,255,.62)' },
  glassOrbLarge: { position: 'absolute', width: 215, height: 215, borderRadius: 108, right: -72, top: -105, backgroundColor: 'rgba(255,255,255,.23)', borderWidth: 1, borderColor: 'rgba(255,255,255,.35)' },
  glassOrbSmall: { position: 'absolute', width: 115, height: 115, borderRadius: 58, left: -43, bottom: -58, backgroundColor: 'rgba(160,205,255,.25)', borderWidth: 1, borderColor: 'rgba(255,255,255,.26)' },
  glassPane: { position: 'absolute', width: 245, height: 95, right: -34, bottom: 14, borderRadius: 25, backgroundColor: 'rgba(255,255,255,.08)', borderWidth: 1, borderColor: 'rgba(255,255,255,.18)', transform: [{ rotate: '-12deg' }] },
  cardTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  chip: { width: 38, height: 29, borderRadius: 8, paddingVertical: 6, justifyContent: 'space-around', backgroundColor: 'rgba(245,222,151,.93)', borderWidth: 1, borderColor: 'rgba(255,255,255,.65)' },
  chipLine: { height: 1, backgroundColor: 'rgba(104,82,19,.52)' }, cardBrand: { color: 'rgba(255,255,255,.88)', fontFamily: FONTS.button, fontSize: 11, letterSpacing: 1.3 },
  cardType: { marginTop: 18, color: 'rgba(226,234,255,.84)', fontFamily: FONTS.button, fontSize: 8, letterSpacing: 1.05 },
  cardNumber: { marginTop: 7, color: '#FFFFFF', fontFamily: FONTS.heading, fontSize: 21, letterSpacing: .8 },
  cardBottom: { marginTop: 'auto', flexDirection: 'row', alignItems: 'flex-end', gap: 22 }, cardMetaLabel: { color: 'rgba(224,232,255,.72)', fontFamily: FONTS.button, fontSize: 7, letterSpacing: .75 }, cardMeta: { marginTop: 3, color: 'white', fontFamily: FONTS.bodyMedium, fontSize: 9 }, visa: { marginLeft: 'auto', color: 'white', fontFamily: FONTS.heading, fontSize: 18, fontStyle: 'italic' },
  actions: { flexDirection: 'row', gap: 10, marginTop: 15 }, action: { flex: 1, minHeight: 75, alignItems: 'center', justifyContent: 'center', gap: 6, borderRadius: 19, backgroundColor: '#EEF3FF', borderWidth: 1.5, borderColor: '#FFFFFF', shadowColor: '#A4B1CF', shadowOpacity: .22, shadowRadius: 7, shadowOffset: { width: 3, height: 4 }, elevation: 3 }, actionPressed: { transform: [{ scale: .96 }], backgroundColor: '#E5ECFF' }, actionIcon: { width: 29, height: 25, alignItems: 'center', justifyContent: 'center' }, actionText: { color: COLORS.ink, fontFamily: FONTS.button, fontSize: 9 },
