...
function RangeSlider({ value, onChange, accent }) {
  ...
  const fillStyle = useAnimatedStyle(() => ({ transform: [{ translateX: -trackWidth.value / 2 * (1 - progress.value / 100) }, { scaleX: progress.value / 100 }] }));
  const thumbStyle = useAnimatedStyle(() => ({ transform: [{ translateX: trackWidth.value * progress.value / 100 - 11 }] }));
  ...
  return <View><View onLayout={(event) => { const nextWidth = event.nativeEvent.layout.width; setWidth(nextWidth); trackWidth.value = nextWidth; }} onStartShouldSetResponder={() => true} onMoveShouldSetResponder={() => true} onResponderGrant={(event) => update(event)} onResponderMove={(event) => update(event)} onResponderRelease={(event) => update(event, true)} style={styles.rangeTouch}><View pointerEvents="none" style={styles.rangeTrack}><Animated.View style={[styles.rangeFill, { backgroundColor: accent }, fillStyle]} /><Animated.View style={[styles.rangeThumb, { borderColor: accent }, thumbStyle]} /></View></View><View style={styles.rangeLabels}><Text style={styles.rangeLabel}>Suave</Text><Text style={[styles.rangeValue, { color: accent }]}>{value}%</Text><Text style={styles.rangeLabel}>Intenso</Text></View></View>;
}
